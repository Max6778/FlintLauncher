...
// Modified: also installs the SDL hooks (native_hooks/sdl_hook.c, ported
// from Amethyst-Android) using the same bytehook_hook_all_p obtained here,
// so we don't need a second bytehook_init() call just for SDL.
//
...
static bool init_exit_hook() {
    ...
    if(bhook_status == BYTEHOOK_STATUS_CODE_OK) {
        ...
        // Install the SDL hooks too (same bytehook instance, same init call).
